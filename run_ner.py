...
def prepare_data(features):
    ...
    if features[0].adj_a and features[0].adj_f:
        adjs_a = torch.tensor([f.adj_a for f in features], dtype=torch.long)
        adjs_f = torch.tensor([f.adj_f for f in features], dtype=torch.long)
        data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_valid_ids,
                             all_lmask_ids, adjs_a, adjs_f)
    elif features[0].adj_a:
        adjs_a = torch.tensor([f.adj_a for f in features], dtype=torch.long)
        data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_valid_ids,
                             all_lmask_ids, adjs_a)
    elif features[0].adj_f:
        adjs_f = torch.tensor([f.adj_f for f in features], dtype=torch.long)
        data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_valid_ids,
                             all_lmask_ids, adjs_f)
    else:
        data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_valid_ids,
                             all_lmask_ids)
...
def evaluate(eval_dataloader,model,label_map,args,tokenizer,device):
    ...
    for batch in tqdm(eval_dataloader,desc="Evaluating"):
        ...
        adj_a, adj_f = None, None
        if args.use_gat and args.gat_type == 'AF':
            input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask, adj_a, adj_f = batch


        elif args.use_gat and args.gat_type == 'A':
            input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask, adj_a = batch


        elif args.use_gat and args.gat_type == 'F':
            input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask, adj_f = batch
        ...
        with torch.no_grad():
            if args.use_crf:
                logits = model(input_ids, segment_ids, input_mask, valid_ids=valid_ids,
                                     attention_mask_label=l_mask, adj_a=adj_a,adj_f=adj_f)
...
def main():
    ...
    model = Ner(args.bert_model,from_tf = False, config = config,tag_to_ix=tag2ix, device=device, use_rnn=args.use_rnn, use_crf=args.use_crf, use_gat=args.use_gat,gat_type=args.gat_type,fuse_type=args.fuse_type, reproject_embeddings=args.relearn_embed)
    ...
    if args.do_train:
        ...
        if args.do_eval:
            ...
            eval_features = convert_examples_to_features(eval_examples, label_list, args.max_seq_length, tokenizer, args.gat_type)
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                batch = tuple(t.to(device) for t in batch)
                adj_a, adj_f = None, None
                # input_ids, input_mask, segment_ids, label_ids, valid_ids,l_mask, pos_ids, dep_ids, head, adj_a, adj_f = batch
                if args.use_gat and args.gat_type == 'AF':
                    input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask, adj_a, adj_f = batch
                elif args.use_gat and args.gat_type == 'A':
                    input_ids, input_mask, segment_ids, label_ids, valid_ids, l_mask, adj_a = batch
                ...
                loss = model(input_ids, segment_ids, input_mask, label_ids, valid_ids, l_mask, adj_a, adj_f)
    ...
    if args.do_predict:
        ...
        test_features = convert_examples_to_features(test_examples, label_list, args.max_seq_length, tokenizer, args.gat_type)
