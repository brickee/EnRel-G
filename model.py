...
class Ner(nn.Module):
    ...
    def __init__(self,
        ...
        self.gat_type: str = gat_type
        self.fuse_type: str = fuse_type
        ...
        if self.use_gat:
            ...
            if 'A' in self.gat_type:
                self.gat_a = GAT(
                    nfeat=hidden_size * 2,
                    nhid=self.nhid,
                    dropout=self.gat_dropout,
                    nheads=self.nheads,
                    alpha=self.alpha, nclass=self.tagset_size)
                if self.fuse_type == 'v':
                    self.weight2 = torch.nn.Parameter(torch.ones(nhid * nheads))
                elif self.fuse_type == 'n':
                    self.weight2 = torch.nn.Parameter(torch.ones(1))

            if 'F' in self.gat_type:
                self.gat_f = GAT(
                    ...
                    alpha=self.alpha, nclass=self.tagset_size)
                if self.fuse_type == 'v':
                    self.weight3 = torch.nn.Parameter(torch.ones(nhid * nheads))
                elif self.fuse_type == 'n':
                    self.weight3 = torch.nn.Parameter(torch.ones(1))

            if self.fuse_type == 'v':
                self.weight1 = torch.nn.Parameter(torch.ones(hidden_size * 2))
            elif self.fuse_type == 'n':
        ...
        if self.use_gat:
            if self.fuse_type == 'm':
                self.linear = torch.nn.Linear(
                    (len(self.gat_type)+1)*nheads * nhid, self.output_size
                )
            else:
                self.linear = torch.nn.Linear(
                    nheads*nhid, self.output_size
                )
    ...
    def forward(self,  input_ids, token_type_ids=None, attention_mask=None, labels=None,valid_ids=None,attention_mask_label=None, adj_a=None, adj_f=None):
        ...
        if self.use_gat:
            ...
            if 'A' in self.gat_type:
                assert (adj_a is not None)
                sequence_output_a = self.gat_a(sequence_output, adj_a)
            if 'F' in self.gat_type:
                assert (adj_f is not None)
                sequence_output_f = self.gat_f(sequence_output, adj_f)

            if self.fuse_type == 'v':
                if self.gat_type == 'AF':
                    sequence_output = torch.mul(sequence_output, self.weight1) + torch.mul(sequence_output_a,
                                                                                           self.weight2) + torch.mul(
                        sequence_output_f, self.weight3)
                elif self.gat_type == 'A':
                    sequence_output = torch.mul(sequence_output, self.weight1) + torch.mul(sequence_output_a,
                                                                                           self.weight2)
                elif self.gat_type == 'F':
                    sequence_output = torch.mul(sequence_output, self.weight1) + torch.mul(sequence_output_f,
                                                                                           self.weight3)

            # n strategy
            if self.fuse_type == 'n':
                if self.gat_type == 'AF':
                    sequence_output = self.weight1 * sequence_output + self.weight2 * sequence_output_a + self.weight3 * sequence_output_f
                elif self.gat_type == 'A':
                    sequence_output = self.weight1 * sequence_output + self.weight2 * sequence_output_a
                elif self.gat_type == 'F':
                    sequence_output = self.weight1 * sequence_output + self.weight3 * sequence_output_f

            # m strategy
            if self.fuse_type == 'm':
                if self.gat_type == 'AF':
                    sequence_output = torch.cat((sequence_output, sequence_output_a, sequence_output_f), dim=2)
                elif self.gat_type == 'A':
                    sequence_output = torch.cat((sequence_output, sequence_output_a), dim=2)
                elif self.gat_type == 'F':
                    sequence_output = torch.cat((sequence_output, sequence_output_f), dim=2)
